...
class AIBSBlock(object):
    def __init__(self,**kwargs):
        ...
        if 'block' in kwargs.keys():    
            ...
            self.block = self.pklToBlock()
            
        self.df = self.createFFfromBlock()
    ...
    def createFFfromBlock(self):
        ...
        for segment in self.block.segments:
            ...
            for i,terrain in enumerate(segment.annotations['terrain']):
                stimType=self.block.annotations['data']['terrain']['params'][i]['name'];
                segmentData[stimType]=terrain
...
#            if no block is input, then it creates a new one
    ...
    for i in range(len(data['terrainlog'])-1):#don't look at the last entry, as it was a lap started but not completed.
       ...
       # for j in range(data['terrain']['params']):
       #     s.annotate(data['terrain']['params'][j]['name']=data['terrainlog'][i][0])
