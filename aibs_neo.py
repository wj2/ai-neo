...
#class that incorporates all of this, including pandas
class AIBSBlock(object):
    def __init__(self,**kwargs):
        if 'file' in kwargs.keys():
            ...
            self.filepath = '/Behavior/Data/M139897/output/140714145627-140714135614-M139897.pkl'
            self.path=os.path.dirname(self.filepath)
        if 'block' in kwargs.keys():    
            self.block = kwargs['block']
        else:
            self.block = self._pklToBlock()
            
        self.df = self._createDFfromBlock()
    ...
    #------------------------------------------------------------------------------
    def _createDFfromBlock(self):
        #make a list of dicts from 
        data = [];
        for segment in self.block.segments:
            segmentData=segment.annotations
            for i,terrain in enumerate(segment.annotations['terrain']):
                segmentData['mouse']=self.block.annotations['data']['mouseid']
                segmentData['date']=self.block.annotations['data']['startdatetime'].partition(':')[0].partition(' ')[0]
                segmentData[self.block.annotations['data']['terrain']['params'][i]['name']]=terrain
                segmentData['licktimes']=segment.spiketrains[0]
    ...
    def toPkl(self,**kwargs):
        ...
        if 'name' in kwargs.keys():
            name = self.filepath.replace('.pkl',''.join(('-',kwargs['name'],'.pkl')))
            #name =''.join((data['stopdatetime'].split(' ')[0].replace('20','').replace('-',''),data['stopdatetime'].split(' ')[1].split('.')[0].replace('-','').replace(':',''),'-',data['startdatetime'].split(' ')[0].replace('20','').replace('-',''),data['startdatetime'].split(' ')[1].split('.')[0].replace('-','').replace(':',''),'-',data['mouseid'],'-',kwargs['name'],'.pkl'))
        else:
   ...
   #------------------------------------------------------------------------------
    def append(self,inpt):
        #how to combine the Block portions? make a list of Blocks? dict of Blocks?
        return pd.concat([self.df,inpt.df])
     #------------------------------------------------------------------------------   
...
#            if no block is input, then it creates a new one
    ...
    for i in range(len(data['terrainlog'])-1):#don't look at the last entry, as it was a lap started but not completed.
        ...
        #optogenetics
        #TODO: make this for real, just placeholders now. 
